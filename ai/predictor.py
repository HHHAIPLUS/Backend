...
class ProductionPredictor:
    """Production predictor backed only by the promoted predictive brain."""
    ...
    def predict(self, features: FeatureVector, regime: MarketRegime) -> Prediction:
        ...
        if not brain.get("trained"):
            return Prediction(symbol=features.symbol, timestamp=datetime.now(timezone.utc), long_probability=0, short_probability=0, no_trade_probability=1, confidence=1, regime=regime, model_version="untrained", rationale=["No promoted predictive brain is available; abstaining."])
        p = brain["probabilities"]
        adaptation = self._adaptive(max(p.values()), brain["version"], regime, features.features)
