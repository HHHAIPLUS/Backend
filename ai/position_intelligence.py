...
class PositionIntelligenceEngine:
    ...
    def thesis_integrity(self, p: PositionObservation, thesis: dict[str, Any]) -> float:
        d=1.0 if p.side.lower()=="long" else -1.0; entry_m=float(thesis.get("momentum",0) or 0); pressure=p.buying_pressure if d>0 else p.selling_pressure
        alignment=self.clamp(.5+d*p.momentum*.5); original=self.clamp(.5+d*entry_m*.5); deterioration=max(0,-d*(p.momentum-entry_m))
        return self.clamp(.22*alignment+.16*original+.22*self.clamp(p.trend_strength)+.20*self.clamp(pressure)+.10*(1-self.clamp(p.news_risk))+.10*(1-self.clamp(p.liquidity_stress))-.20*deterioration)
    def evaluate(self,p: PositionObservation,thesis:dict[str,Any],*,predictive=None,stage5=None,previous_protection=None)->PositionDecision:
        ...
        predictive=predictive or {}; stage5=stage5 or {}; integrity=self.thesis_integrity(p,thesis); probs=predictive.get("probabilities") or {}; dp=float(probs.get("long",0) if p.side.lower()=="long" else probs.get("short",0)); expected=float(stage5.get("expected_value",p.expected_continuation_value) or 0)+(dp-.5)*.04; expected=max(-.20,min(.20,expected))
        adverse_m=max(0,-(p.momentum if p.side.lower()=="long" else -p.momentum)); opposing=p.selling_pressure if p.side.lower()=="long" else p.buying_pressure; retrace=self.clamp((p.peak_return-p.unrealized_return)/p.peak_return) if p.peak_return>0 else 0
        downside=self.clamp(.24*p.volatility+.18*p.liquidity_stress+.18*p.news_risk+.14*p.market_risk+.14*adverse_m+.12*opposing+.10*max(0,-p.unrealized_return*12)); shock=self.clamp(.30*p.news_risk+.25*p.market_risk+.25*p.liquidity_stress+.20*adverse_m); edge=expected-downside*.025; thesis_break=integrity<.38
        if shock>=.86 or p.liquidity_stress>=.94: action,frac,reason="emergency_exit",1.0,"Market-wide or liquidity shock makes continued exposure unsafe."
        elif thesis_break and p.unrealized_return<=0: action,frac,reason="exit",1.0,"The original trade thesis has materially failed while the position is losing."
        elif thesis_break or downside>=.78: action,frac,reason=(("exit",1.0) if p.unrealized_return>.01 else ("reduce",.50)),"Thesis integrity or downside risk has deteriorated beyond continuation value."
        elif p.unrealized_return>0 and (retrace>=.35 or edge<-.005 or shock>=.55): action,frac,reason="reduce",.35,"Profit is available but continuation value has deteriorated; bank part of the gain."
        elif p.unrealized_return>0 and (retrace>=.18 or downside>=.45): action,frac,reason="protect",0.0,"The thesis remains viable, but protection should tighten as risk rises."
        ...
        if action in {"hold","protect","reduce"} and p.unrealized_return>0:
            buf=self.clamp(.001+p.volatility*.006+max(downside,shock)*.003,.001,.018); proposed=p.current_price*(1-buf) if p.side.lower()=="long" else p.current_price*(1+buf); protection=proposed if previous_protection is None else (max(float(previous_protection),proposed) if p.side.lower()=="long" else min(float(previous_protection),proposed))
...
def install_stage6_position_intelligence(trader: Any) -> None:
    ...
    async def manage(self,symbol):
        ...
        else: raw_rows=[{"symbol":p.symbol,"holdSide":p.side,"positionAmt":p.quantity,"entryPrice":p.entry_price,"markPrice":p.mark_price} for p in self.paper.positions.values() if p.symbol.upper()==symbol.upper()]
        ...
        for row in raw_rows:
            ...
            key=f"{exchange}:{ps}:{side}"; world=(await asyncio.to_thread(build_world_intelligence,ps,exchange)).model_dump(mode="json"); market=world.get("market",world); f=market.get("features") or {}; flow=market.get("order_flow") or {}; der=market.get("derivatives") or {}; reg=market.get("regime") or {}; liq=market.get("liquidity") or {}; news=market.get("news") or {}
            if mode=="paper": current=float(f.get("last_price",current) or current); self.paper.mark(ps,current)
            ret=(current-entry)/entry if side=="long" else (entry-current)/entry; peak=max(ret,self.position_peaks.get(key,ret)); self.position_peaks[key]=peak; saved=self._stage6_state.get(key,{})
            ...
            p=PositionObservation(exchange,ps,side,qty,entry,current,ret,peak,.5,float(f.get("momentum",0) or 0),abs(float(f.get("trend_strength",0) or 0)),float(flow.get("aggressive_buy_ratio",.5) or .5),1-float(flow.get("aggressive_buy_ratio",.5) or .5),float(f.get("volatility_proxy",0) or 0),float(f.get("liquidity_stress",0) or liq.get("spread_bps",0)/50),float(news.get("risk",f.get("news_risk",0)) or 0),float(reg.get("market_risk",0) or 0),float(der.get("funding_rate",0) or 0)*100,float(der.get("open_interest_change",0) or 0),0,0,now.isoformat())
            pred=predictive_model.predict(f)
            try: s5=Stage5DecisionEngine(adaptive_intelligence).evaluate(market_state=market,predictive=pred,risk_vetoes=[],position_side=side,unrealized_return=ret,thesis_integrity=.5).as_dict()
            ...
            if now.timestamp()>=cooldown and mode in {"testnet","live"} and d.action in {"exit","emergency_exit"}:
                ...
                pm=await adapter.get_position_mode() if hasattr(adapter,"get_position_mode") else "ONE_WAY"; cq=min(qty,max(.001,self._safe_quantity(qty*d.close_fraction)))
                ...
                old=saved.get("protection_price"); changed=old is None or abs(d.protection_price-float(old))/current>=.0005
                if changed:
                    pm=await adapter.get_position_mode() if hasattr(adapter,"get_position_mode") else "ONE_WAY"
                ...
                if d.action in {"exit","emergency_exit"}: o=self.paper.close(ps,current); action={"status":"closed","order_id":getattr(o,"order_id",None),"mode":"paper"}
                    ...
                    if pp: rq=min(pp.quantity,pp.quantity*d.close_fraction); pp.realized_pnl+=(current-pp.entry_price)*rq*(1 if pp.side=="long" else -1); pp.quantity-=rq; action={"status":"reduced","reduced_quantity":rq,"remaining_quantity":pp.quantity,"mode":"paper"}
            remaining=0.0
