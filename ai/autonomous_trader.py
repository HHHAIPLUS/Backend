...
class AutonomousTrader:
    ...
    async def _execute(self,symbol:str,world:dict[str,Any],decision:dict[str,Any],candidate:TradeCandidate|None,risk:dict[str,Any])->dict[str,Any]:
        ...
        if exchange=='bitget':
            adapter=self._adapters()[exchange]; position_mode=await adapter.get_position_mode(symbol); order={'symbol':symbol,'marginCoin':'USDT','side':side,'orderType':'market','size':safe_qty,'tradeSide':'open','marginMode':'crossed','presetStopLossPrice':f'{stop_price:.8f}','presetStopSurplusPrice':f'{take_profit:.8f}','presetStopLossExecutePrice':'0','presetStopSurplusExecutePrice':'0'}; result=await self.router.place_order(exchange,order,testnet=self.execution_mode=='testnet'); order_id=result.get('orderId') if isinstance(result,dict) else None
