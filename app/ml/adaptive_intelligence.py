...
class AdaptiveIntelligence:
    ...
    def add_observation(self, observation: AdaptiveObservation) -> None:
        if not isfinite(observation.realized_return):
    ...
    def ingest_records(self, records: Iterable[Any]) -> int:
        ...
        for record in records:
            ...
            obs = AdaptiveObservation(
                symbol=str(getattr(record, "symbol", "unknown")),
                model_version=str(getattr(record, "model_version", "unknown")),
                action=str(getattr(record, "action", "NO_TRADE")).upper(),
                confidence=float(getattr(record, "confidence", 0.0)),
                realized_return=float(outcome),
                observed_at=str(getattr(record, "created_at", datetime.now(timezone.utc).isoformat())),
                regime=regime,
                horizon=horizon,
                expected_probability=float(expected) if expected is not None else None,
                features={k: float(v) for k, v in features.items() if isinstance(v, (int, float)) and isfinite(float(v))},
            )
    ...
    def calibration_drift(self, window: int = 200) -> dict[str, float]:
        ...
        if not rows:
            return {"samples": 0.0, "brier": 0.0, "ece": 0.0, "drift": 0.0}
    ...
    def drift_report(self, window: int = 200) -> dict[str, Any]:
        ...
        for key in keys:
            ...
            if a and b: feature_drift[key] = self._psi(a, b)
    ...
    def learned_model_weights(self) -> dict[str, float]:
        ...
        if not eligible:
            return {k: 1.0 / max(1, len(perf)) for k in perf}
