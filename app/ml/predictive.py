...
class PredictiveModel:
    ...
    def predict(
        ...
        missing_price_features = [
            key
            for key in required_price_features
        ...
        if missing_price_features:
            return {
                ...
                "reason": (
                    "Predictive model requires candle-derived features; "
                    "missing: " + ", ".join(missing_price_features)
                ),
            }
    ...
    def load_compact_artifact(
        ...
        if stored_features is not None and list(stored_features) != FEATURES:
            raise ValueError(
                "Model artifact feature ordering does not match the current model."
            )
