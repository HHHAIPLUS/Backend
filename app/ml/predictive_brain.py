...
class PredictiveBrain:
    ...
    def predict(self,features):
        ...
        dm=self.bundle["direction_model"]; bp=dm.predict_proba(x)[0]; er=float(self.bundle["expected_return_model"].predict(x)[0]); dn=max(0.0,float(self.bundle["downside_model"].predict(x)[0])); vo=max(0.0,float(self.bundle["volatility_model"].predict(x)[0])); rg=int(self.bundle["regime_model"].predict(x)[0]); am=1.0 if float(np.max(bp)) < float(self.bundle.get("decision_threshold",0.55)) else 0.0; mf=np.concatenate([bp,np.asarray([er,dn,vo])])[None,:]; mp=self.bundle["meta_model"].predict_proba(mf)[0]; mc=list(self.bundle["meta_model"].classes_); probs={"short":float(mp[mc.index(-1)]) if -1 in mc else 0.0,"flat":float(mp[mc.index(0)]) if 0 in mc else 0.0,"long":float(mp[mc.index(1)]) if 1 in mc else 0.0}; direction=max(probs,key=probs.get); edge=er-self.bundle["cost_rate"]; threshold=float(self.bundle.get("decision_threshold",0.55)); uncertainty=float(1-max(probs.values())); abstain=am>=1.0 or direction=="flat" or max(probs.values())<threshold or edge<=0 or not np.isfinite([er,dn,vo]).all(); return {"trained":True,"abstain":abstain,"version":self.version,"decision":"NO_TRADE" if abstain else direction.upper(),"probabilities":probs,"expected_return":er,"expected_edge_after_cost":edge,"downside":dn,"volatility":vo,"regime":rg,"uncertainty":uncertainty,"abstention_probability":am,"model_family":self.bundle["family"]}
    def manifest(self):
