...
CANONICAL_FEATURES = ("return_1", "range_pct", "volume_change", "order_book_imbalance", "funding_rate", "open_interest_change", "news_risk", "news_sentiment", "volatility_proxy", "trend_strength", "momentum", "liquidity_stress")
...
def _atr_like(bars: Sequence[MarketBar]) -> float:
    ...
    tr = [max(cur.high - cur.low, abs(cur.high - prev.close), abs(cur.low - prev.close)) / prev.close for prev, cur in zip(bars[:-1], bars[1:])]
...
def _timeframe_features(bars: Sequence[MarketBar]) -> dict[str, float | None]:
    ...
    closes = [b.close for b in bars]; rets = _returns(closes); recent = rets[-20:]
...
def _correlation(a: Sequence[float], b: Sequence[float]) -> float | None:
    ...
    ra, rb = _returns(a), _returns(b); n = min(len(ra), len(rb))
    if n < 3: return None
    ra, rb = ra[-n:], rb[-n:]; ma, mb = mean(ra), mean(rb)
...
def point_in_time_join(bars: Sequence[MarketBar], contexts: Sequence[PointInTimeContext], *, max_age_seconds: int = 900) -> list[tuple[MarketBar, PointInTimeContext | None]]:
    ...
    ordered_bars = sorted(bars, key=lambda x: x.timestamp); ordered_context = sorted(contexts, key=lambda x: x.timestamp)
    ...
    for bar in ordered_bars:
        ...
        if latest is None: result.append((bar, None)); continue
        age = (bar.timestamp - latest.timestamp).total_seconds(); result.append((bar, latest if 0 <= age <= max_age_seconds else None))
    return result
...
def assess_quality(state: Mapping[str, object], *, required: Iterable[str], observed_at: datetime, max_age_seconds: int = 60, now: datetime | None = None, threshold: float = 0.80) -> FeatureQuality:
    now = now or datetime.now(timezone.utc); required_list = list(required)
...
def build_market_state(symbol: str, bars_by_timeframe: Mapping[str, Sequence[MarketBar]], context: PointInTimeContext | None, *, correlation_bars: Mapping[str, Sequence[MarketBar]] | None = None, news_count: int = 0, news_sentiment: float = 0.0, news_risk: float = 0.0, news_credibility: float = 0.0, observed_at: datetime | None = None) -> MarketState:
    ...
    primary_tf = "5m" if "5m" in clean else next(iter(clean)); primary = clean[primary_tf]; timestamp = primary[-1].timestamp; observed_at = observed_at or timestamp
    closes = [b.close for b in primary]; rets = _returns(closes); vol = pstdev(rets[-30:]) if len(rets[-30:]) > 1 else 0.0; last = primary[-1]; prev = primary[-2] if len(primary) > 1 else last
    ...
    timeframe_features = {tf: _timeframe_features(bars) for tf, bars in clean.items()}; correlations: dict[str, float | None] = {}
    if correlation_bars:
    ...
    regime_score = max(-1.0, min(1.0, structure["trend"] * 0.7 + max(-1.0, min(1.0, (volume_ratio - 1.0) / 2.0)) * 0.3)); regime = "HIGH_VOLATILITY" if vol > 0.008 else ("TRENDING_UP" if regime_score > 0.45 else "TRENDING_DOWN" if regime_score < -0.45 else "RANGE")
    corr_risk = mean([abs(x) for x in correlations.values() if x is not None]) if correlations else 0.0
    market_risk = max(0.0, min(1.0, vol / 0.02 * 0.5 + corr_risk * 0.2 + news_risk * 0.3))
    ...
    return MarketState(symbol=symbol.upper(), timestamp=timestamp, timeframes=timeframe_features, price_structure=structure, volatility={"realized": vol, "atr": _atr_like(primary)}, volume={"last": last.volume, "mean_20": volume_mean, "ratio": volume_ratio}, order_flow={"imbalance": imbalance, "aggressive_buy_ratio": buy_ratio}, derivatives={"funding_rate": funding, "open_interest": oi, "open_interest_change": None, "liquidation_notional": liq}, liquidity={"spread_bps": spread, "bid_ask_mid": last.close}, correlations=correlations, regime={"label": regime, "score": regime_score, "volatility": vol, "market_risk": market_risk}, news={"count": news_count, "sentiment": news_sentiment, "risk": news_risk, "credibility": news_credibility}, data_quality=quality.score, missing_fields=list(quality.missing), stale_fields=list(quality.stale), usable=quality.usable)
...
def canonical_model_features(state: MarketState) -> dict[str, float]:
    ...
    return {"return_1": float(state.price_structure.get("last_return", 0.0)), "range_pct": float(state.price_structure.get("range_pct_20", 0.0)), "volume_change": float(state.volume.get("ratio", 1.0) - 1.0), "order_book_imbalance": float(state.order_flow.get("imbalance") or 0.0), "funding_rate": float(state.derivatives.get("funding_rate") or 0.0), "open_interest_change": float(state.derivatives.get("open_interest_change") or 0.0), "news_risk": float(state.news.get("risk", 0.0)), "news_sentiment": float(state.news.get("sentiment", 0.0)), "volatility_proxy": float(state.volatility.get("realized", 0.0)), "trend_strength": float(tf5.get("trend") or 0.0), "momentum": float(tf5.get("return") or 0.0), "liquidity_stress": max(0.0, min(1.0, float(state.liquidity.get("spread_bps") or 0.0) / 50.0))}
...
def aggregate_timeframes(bars: Sequence[MarketBar], minutes: int) -> list[MarketBar]:
    ...
    return [MarketBar(symbol=group[0].symbol, timeframe=f"{minutes}m", timestamp=group[0].timestamp, open=group[0].open, high=max(x.high for x in group), low=min(x.low for x in group), close=group[-1].close, volume=sum(x.volume for x in group), quote_volume=sum(x.quote_volume or 0 for x in group)) for group in groups.values() if group]
