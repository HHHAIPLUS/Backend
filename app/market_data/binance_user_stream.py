...
class BinanceUserDataStream:
    ...
    def _api_call(self, method: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        payload = {"id": request_id, "method": method, "params": {"apiKey": self.api_key, **(params or {})}}
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                connected_at = time.monotonic(); next_keepalive = connected_at + 45 * 60
                ...
                with websocket_connect(url, proxy=None, open_timeout=10, close_timeout=5, ping_interval=20, ping_timeout=60, max_size=4 * 2**20) as ws:
                    ...
                    while not self._stop.is_set():
                        if time.monotonic() >= next_keepalive:
                            self._keepalive(); next_keepalive = time.monotonic() + 45 * 60
    ...
    def _apply_account_update(self, payload: dict[str, Any]) -> None:
        ...
        for position in account.get("P") or []:
            ...
            raw_qty = float(position.get("pa") or 0); position_side = str(position.get("ps") or "BOTH").upper()
            side = ("long" if raw_qty > 0 else "short") if position_side == "BOTH" else ("long" if position_side == "LONG" else "short")
            qty = abs(raw_qty); key = f"{symbol}:{side}"
            if qty <= 0:
                self._positions.pop(key, None); continue
    ...
    def _apply_order_update(self, payload: dict[str, Any]) -> None:
        ...
        order["symbol"] = symbol; order["orderId"] = order_id; order["stopPrice"] = float(order.get("sp") or 0); order["status"] = order.get("X"); order["executionType"] = order.get("x")
        key=f"{symbol}:{order_id}"; status=str(order.get("X") or "")
    ...
    def positions(self, symbol: str | None = None) -> list[dict[str, Any]]:
        ...
        if symbol: rows=[row for row in rows if row.get("symbol")==symbol.upper()]
        return rows

    def protection_orders(self, symbol: str) -> list[dict[str, Any]]:
        with self._lock:
            return [dict(row) for row in self._orders.values() if row.get("symbol")==symbol.upper() and str(row.get("o") or row.get("type") or "").upper() in {"STOP_MARKET","TAKE_PROFIT_MARKET","STOP","TAKE_PROFIT","TRAILING_STOP_MARKET"}]

    def cross_wallet_balance(self) -> float:
        with self._lock:
            usdt=self._balances.get("USDT") or {}
            return float(usdt.get("cw") or usdt.get("wb") or 0)

    def is_healthy(self) -> bool:
        with self._lock:
            return self._connected and self._state_initialized and not self._last_error and (time.time()-self._last_message_at <= 90)
