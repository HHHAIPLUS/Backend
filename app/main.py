...
app = FastAPI(title=settings.app_name, version='1.0.0', description='HHHAI backend — cumulative Phase 20', lifespan=lifespan)
...
app.include_router(adversarial_router); app.include_router(model_router); app.include_router(markets_router); app.include_router(trading_router)
...
@app.get('/')
def root():
    return {'name': settings.app_name, 'version': '1.0.0', 'phase': 'Phase 18 - Autonomous Trading Control Center', 'live_trading_enabled': settings.live_trading_enabled, 'execution_authority': False, 'mode': settings.app_env}
