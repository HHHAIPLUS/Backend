...
async def integration_status():
    return {
        'backend':'connected',
        'supabase':{'configured':store.configured,'persistence':store.configured},
        'binance':{'configured':bool(settings.binance_api_key and settings.binance_api_secret),'testnet':settings.binance_testnet},
        'bitget':{'configured':bool(settings.bitget_api_key and settings.bitget_api_secret and settings.bitget_passphrase),'testnet':settings.bitget_testnet},
        'ai_pipeline':'multi-agent + scenario + adversarial + validated-model gate',
        'risk_engine':'fail_closed',
        'testnet_execution':settings.testnet_trading_enabled,
        'live_execution':settings.live_trading_enabled,
        'production_ready':False
    }

@router.get('/bitget/verify')
async def verify_bitget():
    if not (settings.bitget_api_key and settings.bitget_api_secret and settings.bitget_passphrase):
        return {'authenticated':False,'configured':False,'message':'Bitget credentials are not configured'}
    try:
        ...
        return {
            ...
            'autotrading_enabled':settings.autotrading_enabled,
            'message':'Bitget authenticated account check succeeded'
        }
