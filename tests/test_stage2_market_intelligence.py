...
from app.market_data.intelligence import MarketBar, PointInTimeContext, aggregate_timeframes, build_market_state, point_in_time_join
...
def bars(n=60, start=None, step=0.001):
    ...
    return [
        MarketBar(symbol="BTCUSDT", timeframe="5m", timestamp=start + timedelta(minutes=5 * i), open=100 + i * step, high=100.2 + i * step, low=99.8 + i * step, close=100 + (i + 1) * step, volume=1000 + i * 5, quote_volume=100000 + i * 500)
        for i in range(n)
    ]
...
def test_point_in_time_join_never_uses_future_context():
    b = bars(3)
    context = [
        PointInTimeContext(timestamp=b[1].timestamp, funding_rate=0.01),
        PointInTimeContext(timestamp=b[2].timestamp + timedelta(seconds=1), funding_rate=0.99),
    ]
