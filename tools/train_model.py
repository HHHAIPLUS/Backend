...
def load(path: str, horizon: int) -> list[dict]:
    ...
    with open(path, newline="", encoding="utf-8") as f:
        for raw in csv.DictReader(f):
            ...
            rows.append({"observed_at": observed_at, "features": features, "label": label, "outcome_return": outcome_return, "outcome_horizon": horizon, "outcome_return_by_horizon": {str(horizon): outcome_return}, "barrier_return_by_horizon": {str(horizon): outcome_return}, "symbol": raw.get("symbol", ""), "interval": raw.get("interval", ""), "data_source": raw.get("data_source", "csv")})
    return rows
...
def main() -> None:
    ...
    output = {"status": report.status, "version": report.version, "reason": report.reason, "metrics": report.metrics, "artifact": report.artifact, "rows": len(rows)}
